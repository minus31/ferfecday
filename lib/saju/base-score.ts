...
function addDetail(
  details: LuckyScoreDetail[],
  category: LuckyScoreDetail["category"],
  label: string,
  value: number,
  description: string
) {
  if (value === 0) return;
  details.push({ category, label, value: round(value), description });
}
...
function scoreChapter10(
  ...
  if (ELEMENT_KEYS.every((element) => stems.some((stem) => getElement(stem) === element) || branchElements.includes(element))) {
    score += 2;
    addDetail(details, "element-balance", "10장 오행 구족", 2, "원국 오행이 모두 존재합니다.");
  }

  if (presentRoles.has("siksang") && presentRoles.has("jaeseong") && presentRoles.has("gwanseong")) {
    score += 2;
    addDetail(details, "sipseong-structure", "10장 식재관 유기통관", 2, "식상-재성-관성 흐름이 모두 존재합니다.");
  }

  if (sipsinSet.has("偏官") && sipsinSet.has("食神")) {
    score += 2;
    addDetail(details, "sipseong-structure", "10장 식신제살", 2, "편관과 식신이 함께 존재합니다.");
  }

  if (sipsinSet.has("傷官") && (sipsinSet.has("正官") || sipsinSet.has("偏官"))) {
    score -= 1;
    addDetail(details, "sipseong-structure", "10장 상관견관", -1, "상관과 관성이 동시에 존재합니다.");
  }

  if (sipsinSet.has("偏印") && sipsinSet.has("食神") && !presentRoles.has("jaeseong")) {
    score -= 3;
    addDetail(details, "sipseong-structure", "10장 편인도식", -3, "편인과 식신이 함께 있고 재성 제어가 없습니다.");
  }

  const bigeopQi = strength.roleQi.bigeop.percentage;
  if (bigeopQi >= 40 && !hasAnySipsin(result, SIPSIN_BY_ROLE.gwanseong)) {
    score -= 2;
    addDetail(details, "sipseong-structure", "10장 비겁태과 무관", -2, "비겁 기도가 40% 이상이고 관성이 없습니다.");
  }

  if (gender === "F" && !hasAnySipsin(result, SIPSIN_BY_ROLE.gwanseong)) {
    score -= 2;
    addDetail(details, "sipseong-structure", "10장 여아 무관", -2, "여명 기준 관성이 없습니다.");
  }

  if (gender === "M" && !hasAnySipsin(result, SIPSIN_BY_ROLE.jaeseong)) {
    score -= 2;
    addDetail(details, "sipseong-structure", "10장 남아 무재", -2, "남명 기준 재성이 없습니다.");
  }
...
function scoreChapter11(result: SajuResult) {
  ...
  if (["長生", "乾祿", "建祿", "冠帶", "衰"].includes(time.unseong)) {
    score += 0.5;
    addDetail(details, "unseong-sinsal", "11장 시지 길성 안착", 0.5, `시지 12운성 ${time.unseong}`);
  }
  if (["長生", "乾祿", "建祿", "衰"].includes(day.unseong)) {
    score += 0.5;
    addDetail(details, "unseong-sinsal", "11장 일지 길성 배치", 0.5, `일지 12운성 ${day.unseong}`);
  }
  if (daySinsal === "攀鞍" || timeSinsal === "攀鞍") {
    score += 0.5;
    addDetail(details, "unseong-sinsal", "11장 반안살 안착", 0.5, "일지 또는 시지에 반안살이 배치되었습니다.");
  }
  if (["絶", "病", "死"].includes(time.unseong)) {
    score -= 0.5;
    addDetail(details, "unseong-sinsal", "11장 시지 무력", -0.5, `시지 12운성 ${time.unseong}`);
  }
  if (["沐浴", "絶"].includes(day.unseong)) {
    score -= 0.2;
    addDetail(details, "unseong-sinsal", "11장 일지 기복", -0.2, `일지 12운성 ${day.unseong}`);
  }
...
function scoreChapter12(result: SajuResult) {
  ...
  if (cheonulInner > 0) {
    ...
    score += value;
    addDetail(details, "good-bad-stars", "12장 천을귀인 일시지", value, "천을귀인이 일지 또는 시지에 있습니다.");
  }
  if (cheonulOuter > 0) {
    const value = cheonulOuter * 1;
    score += value;
    addDetail(details, "good-bad-stars", "12장 천을귀인 연월지", value, "천을귀인이 연지 또는 월지에 있습니다.");
  }

  if (DAY_JAEGO_PILLARS.has(dayPillar)) {
    score += 1;
    addDetail(details, "good-bad-stars", "12장 재고귀인 일지", 1, `${dayPillar} 재고귀인 일주입니다.`);
  }
  const otherJaego = branches.filter((branch, index) => index !== 1 && JAEGO_BRANCHES.has(branch)).length;
  if (otherJaego > 0) {
    const value = otherJaego * 0.5;
    score += value;
    addDetail(details, "good-bad-stars", "12장 재고귀인 타지", value, "연지/월지/시지에 재고 지지가 있습니다.");
  }

  if (result.specialSals.cheonduk.length > 0 || result.specialSals.wolduk.length > 0) {
    score += 0.2;
    addDetail(details, "good-bad-stars", "12장 천덕, 월덕귀인", 0.2, "천덕 또는 월덕귀인을 보유합니다.");
  }
  if (result.specialSals.munchang.length > 0) {
    score += 0.1;
    addDetail(details, "good-bad-stars", "12장 문창귀인", 0.1, "문창귀인을 보유합니다.");
  }
  if (result.specialSals.geumyeo.length > 0) {
    score += 0.2;
    addDetail(details, "good-bad-stars", "12장 금여록", 0.2, "금여록을 보유합니다.");
  }

  for (const group of [["寅", "巳", "申"], ["丑", "戌", "未"]]) {
    if (group.every((branch) => branches.includes(branch))) {
      score -= 1.5;
      addDetail(details, "good-bad-stars", "12장 삼형살", -1.5, `${group.join("")} 삼형살이 성립합니다.`);
      break;
    }
  }
  ...
  if (cheonra) {
    score -= 1;
    addDetail(details, "good-bad-stars", "12장 천라지망", -1, "일지 또는 월지를 포함한 술해/진사 연속 배치가 있습니다.");
  }
  ...
  if (hasPair(BRANCH_WONJIN, dayBranch, monthBranch) || hasPair(BRANCH_WONJIN, dayBranch, timeBranch) || hasPair(BRANCH_GWIMUN, dayBranch, monthBranch) || hasPair(BRANCH_GWIMUN, dayBranch, timeBranch)) {
    score -= 1.5;
    addDetail(details, "good-bad-stars", "12장 원진귀문", -1.5, "일지와 월지/시지 사이 원진 또는 귀문이 성립합니다.");
  }
  ...
  for (const index of result.gongmang.pillarIndices) {
    ...
    if (["正官", "正印", "正財", "偏財"].includes(sipsin)) {
      score -= 0.5;
      addDetail(details, "good-bad-stars", "12장 길성 공망", -0.5, `${sipsin} 지지가 공망입니다.`);
    } else {
      score -= 0.2;
      addDetail(details, "good-bad-stars", "12장 기타 십성 공망", -0.2, `${sipsin} 지지가 공망입니다.`);
    }
  }
...
export function calculateBaseScoring(
  ...
  const elementBalanceAdjustment = chapter10.details
    .filter((detail) => detail.category === "element-balance")
  ...
  return {
    ...
    breakdown: {
      originalBaseScore: round(strength.baseScore),
      daewoonAverageBaseScore: round(finalBaseScore),
      elementBalanceAdjustment: round(elementBalanceAdjustment),
      sipseongStructure: round(chapter10.score - elementBalanceAdjustment),
      unseongSinsal: round(chapter11.score),
      goodBadStars: round(chapter12.score),
